...
TreePlot$data$DOERN = sapply(TreePlot$data$label, function(x) stringr::str_replace_all(string=x, pattern = "DORN", replacement = "Doern"))
...
#ggtree(TreePlot$data,v size=.5, layout="rectangular")+ geom_tiplab(size=3)+ geom_tippoint(shape=15,size=4, aes(color=factor(visit))) +# + scale_color_viridis_c()
