...
# - Path to unique genes list output by UniqueWithinGenome.py for CC9 (arg 4)
    # /home/acampbe/DFU/data/WGS_2020/cladebreaker/cladebreaker_patient_176/Patient176/Patient176_unique.txt
# - Path to the unique genes list output by UniqueWithinGenome.py for CC8 (arg 5)
# - Path to output scripts for mafft/snp-sites (arg 6)
# - Reference genome (arg 7)
...
if len(argsinput) <7:
    ...
    UniqueGenesList1 = argsinput[3]
    UniqueGenesList2 = argsinput[4]
    OutputScriptDir = argsinput[5]
    ReferenceGenome = argsinput[6]
...
ReferenceFFN = str(ReferenceGenome) + ".ffn"
print(ReferenceFFN)
...
ListOrthologs1 = open(UniqueGenesList1, "r").read().split('\n')
ListOrthologs1.remove('')
ListOrthologs2 = open(UniqueGenesList2, "r").read().split('\n')
ListOrthologs2.remove('')

ListOrthologs=set( ListOrthologs1 + ListOrthologs2 )
print(ListOrthologs)
...
OutputKey = os.path.dirname(UniqueGenesList1)
...
OutputMAFFT = os.path.join(OutputDir, "MAFFT")
os.mkdir(OutputDir)
