...
for(filename in filelist){
  ...
  outputDF = data.frame(NearestNeighbor=df$mash_nearest_neighbor,mobility = df$predicted_mobility, primary_cluster_id=df$primary_cluster_id, IsolateID = genome, df$size)
...
#for(genomeid in unique(FullDF$IsolateID)){
  ...
  #for(i in 1:nrow(SmallDF)){
    #print(i)
    #clustername=toString(SmallDF[i,"primary_cluster_id"])
     #print(i)
    #print(toString(clustername))
...
write.csv(FullDF,file=fullDFfile)
